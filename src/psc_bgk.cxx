...
// ======================================================================
// v_phi_maxwellian
// trying to make a maxwellian for v_phi
...
struct pdist
{
  ...
  Double3 operator()()
  {
    ...
    //double p_y = coef * g.m_e * ((v_phi  / sqrt(1 + 8 * 0.1 * (rho*rho/sqr(get_beta()))))* -z + v_rho * y) / rho;
    //double p_z = coef * g.m_e * ((v_phi /  sqrt(1 + 8 * 0.1 * (rho*rho/sqr(get_beta()))) )* y + v_rho * z) / rho;
  ...
  auto init_np = [&](int kind, Double3 crd, int p, Int3 idx,
    ...
    switch (kind) {
      ...
      case ELECTRON_BACKGROUND:
        ...
        np.p = setup_particles.createMaxwellian(
          {np.kind, np.n, {0, 0, 0}, {.0001, .0001, .0001}, np.tag});
        break;
